...
import time
from lib.crc8_helper import AHT20_crc8_check
...
class AHT20:
    ...
    def __init__(self, addr=0x38, BusNum=1):
        # Initialize AHT20
        self.addr = addr
        self.BusNum = BusNum
        self.cmd_soft_reset()
    ...
    def cmd_soft_reset(self):
        ...
        with SMBus(self.BusNum) as i2c_bus:
            i2c_bus.write_i2c_block_data(self.addr, 0x0, AHT20_CMD_SOFTRESET)
    ...
    def cmd_initialize(self):
        ...
        with SMBus(self.BusNum) as i2c_bus:
            i2c_bus.write_i2c_block_data(self.addr, 0x0 , AHT20_CMD_INITIALIZE)
        return True

    def cmd_measure(self):
        # Send the command to measure
        with SMBus(self.BusNum) as i2c_bus:
            i2c_bus.write_i2c_block_data(self.addr, 0, AHT20_CMD_MEASURE)
    ...
    def get_status(self):
        ...
        with SMBus(self.BusNum) as i2c_bus:
            return i2c_bus.read_i2c_block_data(self.addr, 0x0, 1)[0]
        return True
    ...
    def get_measure(self):
        ...
        with SMBus(self.BusNum) as i2c_bus:
            return i2c_bus.read_i2c_block_data(self.addr, 0x0, 7)
